...
export default function ManageEvent() {
  ...
  const fetchVenues = async () => {
    try {
      ...
      if (!res.ok) {
        Alert.alert("Error", data?.message || "Failed to load venues");
        return;
      }
  ...
  const createVenue = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        Alert.alert("Error", data?.message || "Failed to create venue");
        return;
      }
  ...
  const updateVenue = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`${EVENTS_API}/${editingVenue.id}`, {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${token}`,
        },
      ...
      if (!res.ok) {
        Alert.alert("Error", data?.message || "Failed to update venue");
        return;
      }
  ...
  const deleteVenue = (id) => {
    Alert.alert("Confirm", "Delete this venue?", [
      { text: "Cancel" },
      {
        text: "Delete",
        style: "destructive",
        onPress: async () => {
          try {
            ...
            const data = await res.json().catch(() => ({}));
            if (!res.ok) {
              Alert.alert("Error", data?.message || "Failed to delete venue");
              return;
            }
  ...
  const existingEditingImage = editingVenue?.picture
    ? venueImageUrl(editingVenue.picture)
    : null;
  ...
  const renderVenue = ({ item }) => (
    <View style={styles.card}>
      <View style={styles.cardMediaWrap}>
        {item.picture ? (
          <Image
            source={{ uri: venueImageUrl(item.picture) }}
            style={styles.thumbnail}
          />
        ) : (
          <View style={styles.noThumb}>
            <MaterialIcons name="event" size={22} color={palette.iconGrey} />
      ...
      >
        <View style={styles.page}>
          <View style={styles.header}>
            <TopHeaderBox title="Manage Venue" onBack={() => router.replace("/(tabs)/admin-panel")} />
            ...
            ListEmptyComponent={
              !loading ? (
                <View style={styles.empty}>
                  <MaterialIcons name="event-note" size={30} color={palette.iconGrey} />
                  <Text style={styles.emptyText}>No venues found.</Text>
                  <Text style={styles.emptySub}>
                    Tap <Text style={{ fontFamily: "Poppins_700Bold" }}>Add</Text> to create your first venue.
                  </Text>
                </View>
              ) : null
            }
            ...
            <View style={styles.modalCard}>
              ...
              <View style={styles.modalActions}>
                ...
                <Pressable
                  ...
                  onPress={
                    saving ? undefined : editingVenue ? updateVenue : createVenue
                  }
                >
                  {saving ? (
...
const styles = StyleSheet.create({
  ...
  venueName: { fontSize: 16, fontFamily: "Poppins_700Bold", color: palette.ink },
  thumbnail: {
    width: 120,
    height: 76,
    borderRadius: 16,
