...
export default function CourtDashboard() {
  ...
  const fetchBookings = async () => {
    ...
    try {
      ...
      const res = await axios.get(`${API_BASE}/bookings/admin/all`, {
  ...
  const saveEdit = async () => {
    ...
    try {
      await axios.put(
        `${API_BASE}/bookings/admin/${selected.id}/status`,
        { status: editBookingStatus },
        { headers: { Authorization: `Bearer ${token}` } }
      );

      await axios.put(
        `${API_BASE}/bookings/admin/${selected.id}/payment`,
  ...
  const ListHeader = (
    <View>
      <TopHeaderBox title="Court Dashboard" onBack={() => router.replace("/(tabs)/admin-panel")} />

      <View style={styles.topControls}>
        <View style={styles.searchBar}>
          <FontAwesome5 name="search" size={14} color="#97A3B7" />
          <Text style={styles.searchGhost}>
            {search || "Search user or court"}
          </Text>
          {!!search && (
      ...
      <TouchableOpacity activeOpacity={1} style={styles.hiddenInputOverlay}>
        <View style={styles.hiddenInputInner}>
          <FontAwesome5 name="search" size={14} color="#97A3B7" />
          <Text style={styles.hiddenInputText}>{search || "Search user or court"}</Text>
        </View>
      </TouchableOpacity>
          ...
          <View style={styles.modalOverlay}>
            <View style={styles.modal}>
              ...
              <View style={styles.modalInfoCard}>
                ...
                <InfoRow label="Court" value={selected.court_name || "-"} />
                <InfoRow
                  label="Booking Date"
...
function TextNativeInput({ value, onChangeText }) {
  ...
  return (
    <TextInput
      ...
      placeholder="Search user or court"
